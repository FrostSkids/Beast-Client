    ...
    public void render(final ScreenPosition pos) {
        if (ModToggle.loadEnabledFromFile("ModPosition")) {
            if (!ModToggle.loadEnabledFromFile("Chroma")) {
                ...
                Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(ModY.color() + "[§fZ" + ModY.color() + "] " + "§f" + Math.round(Minecraft.getMinecraft().thePlayer.posZ * 1000.0) / 1000L, (float)pos.getAbsoluteX(), (float)pos.getAbsoluteY(), -1);
