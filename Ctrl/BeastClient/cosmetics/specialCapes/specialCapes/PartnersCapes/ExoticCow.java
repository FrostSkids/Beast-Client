...
package Ctrl.BeastClient.cosmetics.specialCapes.specialCapes.PartnersCapes;
...
public class ExoticCow implements LayerRenderer<AbstractClientPlayer>
{
    private final RenderPlayer playerRenderer;
    private static final String __08FID = "CL_00002425";
    
    public ExoticCow(final RenderPlayer playerRendererIn) {
    ...
    public void doRenderLayer(final AbstractClientPlayer entitylivingbaseIn, final float p_177141_2_, final float p_177141_3_, final float partialTicks, final float p_177141_5_, final float p_177141_6_, final float p_177141_7_, final float scale) {
        final String s = EnumChatFormatting.getTextWithoutFormattingCodes(entitylivingbaseIn.getName());
        if (s.equals("ExoticCow") && entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername())) {
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.playerRenderer.bindTexture(new ResourceLocation("cape5.png"));
