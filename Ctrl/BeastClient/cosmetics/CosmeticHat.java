...
public class CosmeticHat extends Cosmetic
{
    ...
    @Override
    public void render(final AbstractClientPlayer entitylivingbaseIn, final float limbSwing, final float limbSwingAmount, final float partialTicks, final float ageInTicks, final float headYaw, final float headPitch, final float scale) {
        if (!entitylivingbaseIn.isInvisible() && CosmeticToggle.loadEnabledFromFile("CosmeticHat") && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername())) {
            GlStateManager.pushMatrix();
            final float f = partialTicks;
            final float f2 = this.getFirstRotationX(entitylivingbaseIn, f);
            final float f3 = this.getSecondRotationX(entitylivingbaseIn, f);
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.setRotationAngles(limbSwing, limbSwingAmount, partialTicks, ageInTicks, headYaw, headPitch, entitylivingbaseIn);
